...
void
PropertyUtil::write(const char* filename, PropertyMapT& map,
                    const char* header)
{
...
std::string
PropertyUtil::combineArgs(std::vector<std::string> args, unsigned int start)
{
    std::string str;
    for (unsigned int x = start; x < args.size(); ++x)
    {
        if (x > start)
        {
            str += ' ';
